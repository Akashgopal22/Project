...
def db_connect():
    return mysql.connector.connect(host = "localhost", user = "root", password = "", database = "library")
        ...
        if self.path == '/read':
            ...
            try:
                if db_connect().is_connected():
                    ...
                    self.send_header('Access-Control-Allow-Origin', '*')
                    self.end_headers()
                    self.wfile.write(bytes(group(db_connect()), "utf-8"))
        ...
        if self.path == '/create':
            ...
            try:
                if db_connect().is_connected():
                    ...
                    self.end_headers()
                    # call delete function from delete file
                    self.wfile.write(bytes(getid(db_connect(), req_data), "utf-8"))
